...
lines (Pol xs@(x:_)) = groupConsecutive xs x
 where 
  groupConsecutive [] _ = error "expected nonempty list"
  groupConsecutive (x1:[]) f    = [(x1,f)]
  groupConsecutive (x1:x2:xs) f = (x1,x2) : lineHelper (x2:xs) f
  

contains::Shape->Point->Bool
contains (Circ (c,r)) p = norm c p < r
